-- Add preorder support to product variants
ALTER TABLE product_variants
  ADD COLUMN IF NOT EXISTS preorder_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS preorder_limit   integer DEFAULT NULL;

-- Core preorders table
CREATE TABLE IF NOT EXISTS preorders (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  order_number      text NOT NULL UNIQUE,
  source            text NOT NULL DEFAULT 'online' CHECK (source IN ('online', 'popup')),
...
CREATE INDEX IF NOT EXISTS idx_preorders_variant ON preorders(variant_id);
...
-- Authenticated users can view their own preorders
