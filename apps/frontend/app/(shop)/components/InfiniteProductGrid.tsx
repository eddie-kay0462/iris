            ...
            <div className="grid grid-cols-2 gap-x-0.5 gap-y-10 sm:grid-cols-3 lg:grid-cols-4">
                {Array.from({ length: 8 }).map((_, i) => (
                    <div key={i} className="space-y-3">
                        ...
                        <div className="h-3 w-1/4 animate-pulse rounded bg-fill" />
                    </div>
                ))}
            </div>
        );
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
                <p className="text-[13px] uppercase tracking-[0.14em] text-text-secondary">
                    Something went wrong loading products.
                </p>
                <button
                    onClick={() => window.location.reload()}
                    className={`px-6 py-2.5 ${outlineButton}`}
                >
                    Try again
                </button>
            </div>
    ...
    return (
        <>
            ...
            {hasNextPage && (
                ...
                >
                    {isFetchingNextPage ? (
                        <div className="h-6 w-6 animate-spin rounded-full border-2 border-line-strong border-t-invert-bg" />
                    ) : (
