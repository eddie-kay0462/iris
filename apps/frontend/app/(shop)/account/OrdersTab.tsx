      ...
      {isLoading ? (
        <div className="border-t border-line mt-8">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-[52px] border-b border-line-subtle bg-surface-subtle"
            />
          ))}
        </div>
      ) : orders.length === 0 ? (
        ...
        <div className="border-t border-line mt-8">
          {orders.map((order) => (
            <div key={order.id}>
              <div
                className="grid grid-cols-[1fr_auto_auto_20px] gap-6 max-sm:gap-3 items-center py-4 cursor-pointer border-b border-line-subtle transition-colors duration-150 hover:bg-surface-subtle"
              ...
              >
                ...
                <div className="text-[13px] font-medium text-right min-w-[80px] text-text">
                  {fmt(order.total)}
                </div>
                <svg
                  width="12"
                  ...
                  strokeWidth="2"
                  strokeLinecap="round"
                  className={`text-text flex-shrink-0 transition-transform duration-[250ms] ${expanded === order.id ? "rotate-180" : ""}`}
                  aria-hidden="true"
                >
                  <path d="m6 9 6 6 6-6" />
                </svg>
                    ...
                    {(order.order_items ?? []).map((item, i) => {
                      ...
                      return (
                        <div key={item.id ?? i} className="flex gap-3.5 items-center">
                          <div
                            className="w-14 h-[70px] flex-shrink-0 bg-fill bg-cover bg-center bg-no-repeat"
                            style={imgUrl ? { backgroundImage: `url(${imgUrl})` } : undefined}
                          />
                          <div className="flex-1 min-w-0">
                            <div className="text-[12px] font-medium uppercase tracking-[0.04em] text-text">
                              {item.product_name}
                            </div>
                            <div className="text-[11px] text-text-muted mt-0.5">
