        ...
        <div key={method} className="space-y-1">
          <div className="flex items-center justify-between text-sm">
            <span className="flex items-center gap-2 font-medium text-slate-800">
              <span className={`h-2.5 w-2.5 rounded-full ${colors[method] ?? "bg-slate-400"}`} />
    ...
    <div className="flex flex-wrap gap-2">
      {Object.entries(data).map(([status, count]) => (
        ...
        >
          ...
          <span className="rounded-full bg-white/60 px-1.5 py-0.5 font-bold">{count}</span>
        </div>
      ))}
    </div>
  );
...
function AnalyticsBody({ data }: { data: PopupAnalytics }) {
  ...
  return (
    <div className="space-y-6 print-body">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-xs text-slate-400">
                AOV across {existingCustomer.orderCount} orders · {fmt(existingCustomer.revenue)} total
              </p>
              <p className="text-xs text-slate-500">Customers who provided contact details</p>
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard title="New / Walk-in Customers">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-xs text-slate-400">
                AOV across {newCustomer.orderCount} orders · {fmt(newCustomer.revenue)} total
              </p>
              <p className="text-xs text-slate-500">Walk-in purchases with no contact captured</p>
            </div>
          </div>
            ...
            >
              {existingCustomer.aov >= newCustomer.aov ? (
                <>
                  ...
                  <strong>
                    {pct(((existingCustomer.aov - newCustomer.aov) / newCustomer.aov) * 100)}
                  </strong>{" "}
                ...
                <>
                  ⚠️ Walk-ins spend{" "}
                  <strong>
                    {pct(((newCustomer.aov - existingCustomer.aov) / existingCustomer.aov) * 100)}
                  </strong>{" "}
      ...
      {(Object.keys(revenueByHour).length > 0 || Object.keys(ordersByHour).length > 0) && (
      ...
      <SectionCard title="Pricing Sensitivity & Discount Impact">
        <div className="grid gap-6 sm:grid-cols-3">
          <div className="space-y-1">
            ...
            <p className="text-xl font-bold text-slate-900">{discountImpact.fullPriceCount}</p>
            <p className="text-sm font-semibold text-slate-700">{fmt(discountImpact.fullPriceRevenue)}</p>
          </div>
          <div className="space-y-1">
            <p className="text-xs text-slate-400">Discounted Orders</p>
            <p className="text-xl font-bold text-slate-900">{discountImpact.discountedCount}</p>
            <p className="text-sm font-semibold text-slate-700">{fmt(discountImpact.discountedRevenue)}</p>
            {discountImpact.avgDiscountPct > 0 && (
              <p className="text-xs text-slate-400">Avg. discount: {pct(discountImpact.avgDiscountPct)}</p>
            )}
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <div className="h-3 w-full rounded-full bg-slate-100">
              <div
                ...
                style={{ width: `${(discountImpact.fullPriceRevenue / totalDiscountRevenue) * 100}%` }}
              />
            </div>
          </div>
          <div className="space-y-1">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="pb-2 text-left text-xs font-medium uppercase tracking-wide text-slate-400">Name</th>
                  <th className="pb-2 text-left text-xs font-medium uppercase tracking-wide text-slate-400">Phone</th>
                  <th className="pb-2 text-left text-xs font-medium uppercase tracking-wide text-slate-400">Email</th>
                  <th className="pb-2 text-right text-xs font-medium uppercase tracking-wide text-slate-400">Total Spend</th>
                </tr>
              </thead>
                ...
                {customerCapture.map((c, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    ...
                    <td className="py-2.5 text-right font-medium text-slate-800">
                      {c.totalSpend > 0 ? fmt(c.totalSpend) : <span className="text-slate-300">—</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
...
export function PopupsView() {
  ...
  useEffect(() => {
    if (!selectedId && events && events.length > 0) {
