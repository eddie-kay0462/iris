...
export default function PhoneInput({
  ...
  const nsn = value.startsWith(activeEntry.dial)
    ...
    : value;
  const displayValue =
    nsn && activeEntry.trunk && !nsn.startsWith(activeEntry.trunk)
  ...
  function handleCountryChange(code: string) {
    ...
    setSelected(entry);
    const localDigits = nsn
      .replace(/\D/g, "")
      .replace(new RegExp(`^${entry.trunk}`), "");
    ...
    <div className={className}>
      <div className="flex">
        <select
          ...
          className="rounded-l-md border border-r-0 border-neutral-300 bg-neutral-50 px-2 py-2 text-sm text-neutral-700 outline-none transition focus:border-neutral-900 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-300"
        >
          {DIAL_CODES.map((d) => (
        ...
        <input
          ...
          required={required}
          className="flex-1 rounded-r-md border border-l-0 border-neutral-300 px-3 py-2 text-sm outline-none transition focus:border-neutral-900 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white"
        />
      </div>
